#!/usr/bin/env python3
"""
Angular Frontend Docker Management CLI
Rich UI-based container management for Angular 18 application with tagging support.
"""
import typer
import subprocess
import json
...
app = typer.Typer(
    name="angular",
    help="🅰️ Angular frontend container management with rich UI",
    add_completion=False
)
console = Console()

# Configuration
IMAGE_BASE_NAME = "take-your-time-frontend"
DEFAULT_PORT = 4200
...
def get_angular_images() -> List[Dict[str, str]]:
    """Get list of Angular images."""
...
def get_angular_containers() -> List[Dict[str, str]]:
    """Get list of Angular containers."""
...
def display_images_table(images: List[Dict[str, str]]) -> None:
    ...
    if not images:
        console.print("[yellow]No Angular images found[/yellow]")
        return
    
    table = Table(title="Angular Frontend Images")
...
def display_containers_table(containers: List[Dict[str, str]]) -> None:
    ...
    if not containers:
        console.print("[yellow]No Angular containers found[/yellow]")
        return
    
    table = Table(title="Angular Frontend Containers")
    table.add_column("№", style="cyan", no_wrap=True)
...
def build_image(
    ...
    """🏗️ Build Angular Frontend Docker image."""
    console.print(Panel.fit(f"🏗️ Building Angular Frontend Image: {IMAGE_BASE_NAME}:{tag}", style="blue"))
    
    if not FRONTEND_PATH.exists():
        console.print(f"[red]❌ Frontend path not found: {FRONTEND_PATH}[/red]")
        return
    
    if not (FRONTEND_PATH / "Dockerfile").exists():
        console.print(f"[red]❌ Dockerfile not found in: {FRONTEND_PATH}[/red]")
        return
    
    image_name = f"{IMAGE_BASE_NAME}:{tag}"
    console.print(f"[blue]🔨 Building image {image_name}...[/blue]")
    console.print(f"[dim]Building from: {FRONTEND_PATH}[/dim]")
    
    result = run_command(f"docker build -t {image_name} --target production .", capture_output=False, cwd=FRONTEND_PATH)
    
    if result and result.returncode == 0:
        console.print(f"[green]✅ Successfully built image: {image_name}[/green]")
        console.print(f"[blue]💡 Run with: angular run --tag {tag}[/blue]")
        ...
        if Confirm.ask("\nDo you want to run a container with this image now?", default=False):
            ...
            if result_check and result_check.stdout.strip():
                ...
                if Confirm.ask("Remove existing container?", default=True):
                    ...
                    new_name = Prompt.ask("Enter a different container name", default=f"angular-{tag}-new")
                    container_name = new_name
            
            # Run the container
            command = f"docker run -d --name {container_name} -p {port}:80 {image_name}"
            ...
            if result_run and result_run.returncode == 0:
                ...
                console.print(f"[blue]🌐 Frontend available at: http://localhost:{port}[/blue]")
...
def run_container(
    ...
    port: int = typer.Option(DEFAULT_PORT, "--port", "-p", help="Host port to bind to"),
    ...
    """🚀 Run Angular Frontend container."""
    image_name = f"{IMAGE_BASE_NAME}:{tag}"
    container_name = name or f"angular-{tag}"
    
    console.print(Panel.fit(f"🚀 Running Angular Frontend Container: {container_name}", style="green"))
    
    # Check if image exists
    result = run_command(f"docker images -q {image_name}")
    if not result or not result.stdout.strip():
        console.print(f"[red]❌ Image {image_name} not found[/red]")
        console.print(f"[blue]💡 Build it first with: angular build --tag {tag}[/blue]")
        return
    ...
    command = f"docker run {detach_flag} --name {container_name} -p {port}:80 {image_name}"
...
def remove_containers():
    """🗑️ Remove Angular Frontend containers with interactive selection."""
    console.print(Panel.fit("🗑️ Remove Angular Frontend Containers", style="red"))
    
    containers = get_angular_containers()
    if not containers:
        console.print("[yellow]No Angular containers found[/yellow]")
        return
...
def remove_images():
    """🗑️ Remove Angular Frontend images with interactive selection."""
    console.print(Panel.fit("🗑️ Remove Angular Frontend Images", style="red"))
    
    images = get_angular_images()
    if not images:
        console.print("[yellow]No Angular images found[/yellow]")
        return
...
@app.command("list")
def list_resources():
    """📋 List all Angular Frontend images and containers."""
    console.print(Panel.fit("📋 Angular Frontend Resources", style="blue"))
...
def show_logs(
    ...
    """📋 Show container logs."""
    console.print(Panel.fit("📋 Angular Frontend Container Logs", style="blue"))
...
@app.command("status")
def status():
    """📊 Show Angular Frontend container status."""
    console.print(Panel.fit("📊 Angular Frontend Status", style="blue"))
...
@app.command("clean")
def clean():
    """🧹 Clean up all Angular Frontend containers and images."""
    console.print(Panel.fit("🧹 Clean Angular Frontend Resources", style="red"))
    
    # Stop and remove all containers
    containers = get_angular_containers()
    if containers:
        console.print("[yellow]Stopping and removing all Angular containers...[/yellow]")
        for container in containers:
            console.print(f"[blue]🛑 Stopping {container['names']}...[/blue]")
            run_command(f"docker stop {container['id']}")
            console.print(f"[blue]🗑️  Removing {container['names']}...[/blue]")
            run_command(f"docker rm {container['id']}")
    
    # Remove all images
    images = get_angular_images()
    if images:
        console.print("[yellow]Removing all Angular images...[/yellow]")
        for image in images:
            image_ref = f"{image['repository']}:{image['tag']}"
            console.print(f"[blue]🗑️  Removing {image_ref}...[/blue]")
            run_command(f"docker rmi {image['id']}")
